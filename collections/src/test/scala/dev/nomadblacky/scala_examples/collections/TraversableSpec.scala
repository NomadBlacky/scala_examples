package dev.nomadblacky.scala_examples.collections
...
class TraversableSpec extends AnyFunSpec {
  ...
  it("splitAt ... コレクションを分割する") {
    val expect = (List(1, 2), List(3, 4))
    assert(List(1, 2, 3, 4).splitAt(2) == expect)
  }
  ...
  it("partition ... 条件を満たす要素とそうでない要素に分割する") {
    val expect1 = (List(1, 2), List(3, 4, 5))
    assert(List(1, 2, 3, 4, 5).partition(_ < 3) == expect1)

    val expect2 = (List(4, 5), List(1, 2, 3))
    assert(List(1, 2, 3, 4, 5).partition(_ > 3) == expect2)
  }

  it("span ... 条件がfalseとなった要素を堺にコレクションを分割する") {
    val expect1 = (List(1, 2), List(3, 4, 5))
    assert(List(1, 2, 3, 4, 5).span(_ < 3) == expect1)

    val expect2 = (List(), List(1, 2, 3, 4, 5))
    assert(List(1, 2, 3, 4, 5).span(_ > 3) == expect2)
  }
  ...
  it("unzip ... 要素を2つのコレクションに分割する") {
    val expect1 = (List(1, 2, 3), List("one", "two", "three"))
    assert(List((1, "one"), (2, "two"), (3, "three")).unzip == expect1)

    val expect2 = (List(1, 2, 3), List("one", "two", "three"))
    assert(Map(1 -> "one", 2 -> "two", 3 -> "three").unzip == expect2)
  }
  ...
  it("unzip3 ... 要素を3つのコレクションに分割する") {
    ...
    assert(List((1, "one", "hoge"), (2, "two", "foo"), (3, "three", "bar")).unzip3 == expect)
  }
  ...
  it("toArray ... コレクションをArrayに変換する") {
    ...
    assert(List(1, 2, 3).toArray sameElements Array(1, 2, 3))
  }
  ...
  it("view/force ... コレクションの操作を遅延評価させる(中間データを作らない)") {
    assert(List(1, 2, 3).view.map(_ * 2).toList == List(2, 4, 6))
  }
