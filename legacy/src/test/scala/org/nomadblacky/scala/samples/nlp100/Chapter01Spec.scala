...
/** 言語処理100本ノック 第1章: 準備運動
  ...
  */
class Chapter01Spec extends AnyFunSpec with org.scalatest.matchers.should.Matchers {
  ...
  it("02. 「パトカー」＋「タクシー」＝「パタトクカシーー」") {
    ...
    // A01
    val result = (str1 zip str2).flatMap { case (c1, c2) => Seq(c1, c2) }.mkString
  ...
  it("03. 円周率") {
    ...
    str.replaceAll(""",|\.""", "").split(" ").foreach { word =>
      buffer += word.length()
    }
  ...
  it("04. 元素記号") {
    ...
    /** "Hi He Lied Because Boron Could Not Oxidize Fluorine. New Nations Might Also Sign Peace Security Clause. Arthur
      * King Can." という文を単語に分解し，1, 5, 6, 7, 8, 9, 15, 16, 19番目の単語は先頭の1文字 それ以外の単語は先頭に2文字を取り出し
  ...
  it("08. 暗号文") {
    ...
    def chpher01(text: String): String =
      text.map { c =>
  ...
  it("09. Typoglycemia") {
    ...
    def typoglycemia03(str: String): String = {
      ...
      arr
        .map { text =>
          if (text.length > 4) randomize(text) else text
        }
        .mkString(" ")
    }
