...
/** Created by blacky on 17/05/23.
  ...
  * 参考
  *
  * + 型クラスをOrderingを用いて説明してみる + http://kmizu.hatenablog.com/entry/2017/05/22/224622 +
  * Scalaの型クラス（typeclass）の分かりやすい説明(Cats公式翻訳) + http://qiita.com/mizunowanko/items/7d8658c765567677b280 + Scalaで型クラス入門 +
...
class TypeClassSpec extends AnyFunSpec {
  ...
  it("型クラスとは") {
    ...
    /** + 型クラスとは + ある型に振る舞いを提供する仕組み。 + 「アドホック多相」を実現する。 + 引数の型に応じて複数の実装を提供できる。 + 型の宣言時に振る舞いを与えず、アドホック(場当たり的に)実装を提供できる。 +
      * (特にScalaでは)異なるスコープにおいて、型クラスの実装を有効・無効にできる。 + 複数の型に対して共通の振る舞いをもたせる、という意味の用途はinterfaceとほぼ変わらない。 +
      * が、interfaceにはない利点が多い + interfaceは元のクラス定義を書き換える必要がある。 (ex. Stringに新しいinterfaceを実装することはできない。) +
      * ある型に対する操作を後付けで加えることができる。 + 型クラスのインスタンス(OPPのインスタンスとは異なる)を作る ≒ Strategyパターンの具体的なStrategyを定義する
      */
  }
  ...
  it("型クラスの例") {
    ...
    /** implicit parameter によって、Orderingの引数に与えられる
      ...
      *
      * trait IntOrdering extends Ordering[Int] { def compare(x: Int, y: Int) = java.lang.Integer.compare(x, y) }
      * implicit object Int extends IntOrdering
      *
      * trait StringOrdering extends Ordering[String] { def compare(x: String, y: String) = x.compareTo(y) } implicit
